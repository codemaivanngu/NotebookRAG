...
def app():
    def get_context_retriever_chain(db):
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                ...
                ),  # why placeholder: if not exist chat_history: -> None else chat_history
                ("user", "{input}"),
                (
                    "user",
    ...
    # vector_store need to be persistent: in order to create vector_store
    # we need to use embedding model -> avoid waste resource (we don't want
    # to re-embedding the same content every single time)
